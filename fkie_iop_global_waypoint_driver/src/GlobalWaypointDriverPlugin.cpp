...
#include "GlobalWaypointDriverPlugin.h"
...
GlobalWaypointDriverPlugin::GlobalWaypointDriverPlugin()
{
	p_my_service = NULL;
	p_base_service = NULL;
...
JTS::Service* GlobalWaypointDriverPlugin::get_service()
{
	return p_my_service;
}

void GlobalWaypointDriverPlugin::create_service(JTS::JausRouter* jaus_router)
{
	p_base_service = static_cast<ManagementService *>(get_base_service());
	...
	p_my_service = new GlobalWaypointDriverService(jaus_router, p_transport_service, p_events_service, p_accesscontrol_service, p_base_service);
}

PLUGINLIB_EXPORT_CLASS(iop::GlobalWaypointDriverPlugin, iop::PluginInterface)
